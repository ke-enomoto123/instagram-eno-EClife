...
{{
  ...
  "happy_scene": "Yahoo!ショッピング・PayPayで得しているユーザーの様子（英語で、イラスト指示として）",
  "sad_scene": "知らない人が損している様子（英語で）",
  "visual_element": "お得さを表す装飾要素（英語で。例: golden coins, shopping bags flying, star burst effects）"
}}"""
        }]
    )
...
def _build_infographic_prompt(caption: str) -> str:
    ...
    try:
        ...
        title = concept.get("title", "Yahoo!ショッピングで賢く節約！")
        key_number = concept.get("key_number", "")
        happy_scene = concept.get("happy_scene", "a cheerful person holding shopping bags with coins flying around, big smile")
        sad_scene = concept.get("sad_scene", "a confused sad person looking at expensive receipt with empty wallet")
        visual_element = concept.get("visual_element", "golden coins, shopping bags, star bursts, upward arrows")
        print(f"[Image] ビジュアルコンセプト: {concept}")
    except Exception as e:
        print(f"[Image] キャプション分析失敗、デフォルト使用: {e}")
        title = "ECで損してない？賢い買い方教えます"
        key_number = ""
        happy_scene = "a cheerful person surrounded by delivered packages and coins, big smile, holding smartphone"
        sad_scene = "a confused sad person looking at expensive shopping receipt with empty wallet"
        visual_element = "golden coins, shopping bags flying, star burst effects, upward arrow graph"
...
Scene layout:
- LEFT HALF: {happy_scene}. Label below in Japanese bold text: "賢いユーザー✨"
...
Make it instantly communicate: "この仕組みを知っているかどうかで差がつく！"
