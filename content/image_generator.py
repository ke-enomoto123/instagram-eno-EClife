...
画像はYahoo!ショッピング・PayPayカード・LYPプレミアムなどのEC節約情報を訴求するもので、
...
{{
  ...
  "featured_service": "メインで見せるサービス名（例: Yahoo!ショッピング, PayPayカード, LYPプレミアム）",
  "event_trigger": "日付・期間・イベント名があれば（例: 5のつく日, 週末限定, 今月末まで。なければ空文字）",
  "visual_objects": "画像内に描くべきオブジェクト（英語で。例: smartphone showing Yahoo Shopping app, delivered packages, golden coins, PayPay card, shopping bags）",
...
def _build_infographic_prompt(caption: str) -> str:
    ...
    try:
        ...
        main_headline  = c.get("main_headline", "ECで損してない？")
        sub_headline   = c.get("sub_headline", "Yahoo!ショッピングで賢く節約")
        key_stat       = c.get("key_stat", "")
        featured       = c.get("featured_service", "Yahoo!ショッピング")
        event_trigger  = c.get("event_trigger", "")
        visual_objects = c.get("visual_objects", "smartphone showing Yahoo Shopping app, delivered packages, golden coins, shopping bags")
        mood           = c.get("mood", "excited")
        print(f"[Image] 広告コンセプト: {c}")
    except Exception as e:
        print(f"[Image] キャプション分析失敗、デフォルト使用: {e}")
        main_headline  = "ECで損してない？"
        sub_headline   = "Yahoo!ショッピングで賢く節約"
        key_stat       = ""
        featured       = "Yahoo!ショッピング"
        event_trigger  = ""
        visual_objects = "smartphone showing Yahoo Shopping app, delivered packages, golden coins, red shopping bag"
        mood           = "excited"
